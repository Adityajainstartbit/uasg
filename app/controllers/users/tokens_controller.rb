...
module Devise
  module Api
    class Users::TokensController < Devise.api.config.base_controller.constantize
      ...
      def sign_up
        ...
        if service.success?
          ...
          roles = token.resource_owner.roles.pluck(:name)
          role = roles.present? ? roles.first : nil
          ...
          return render json: token_response.body.as_json.merge(role: role), status: token_response.status
        end
      ...
      def info
        token_response = Devise::Api::Responses::TokenResponse.new(request, token: current_devise_api_token,
                                                                            action: __method__)
        roles = token_response.resource_owner.roles.pluck(:name)
        role = roles.present? ? roles.first : nil

        render json: token_response.body.as_json.merge(role: role), status: token_response.status
      end
